...
class NotifyHandler(base_handler.BaseHandler):
    ...
    def post(self):
        ...
        if util.make_alipy_sign(args, self.settings["alipay_secret"]) != sign:
            LOGGER.error("web_yymoon@sign is not valid")
            self.response("error")
            return
        
        if args.get("trade_status") != "TRADE_SUCCESS" and args.get("trade_status") != "TRADE_FINISHED":
            LOGGER.error("web_yymoon@trade_status is not success")
            self.response("error")
            return

        try:
            pay = self.db.get("select id from pay_t where orderid = %s and pt = 'web_yymoon'", args.get("trade_no"))
            if pay:
                LOGGER.warning("web_yymoon@repeat send")
                self.response("success")
                return
        except Exception, e:
            LOGGER.error("web_yymoon@%s", e)
            self.response("error")
            return
        ...
        if check == 0:
            try:
                self.db.insert("insert into pay_t (id, openid, userid, itemid, orderid, amount, res, pt, dt) values (0, %s, %s, %s, %s, %s, %s, %s, NOW())",
                                            args.get("extra_common_param"), 0, 0, args.get("trade_no"), int(float(args.get("total_fee")) * 100), 0, "web_yymoon")
            except Exception, e:
                LOGGER.error("web_yymoon@%s", e)
                self.response("error")
                return
        else:
            LOGGER.error("web_yymoon@verify error")
            self.response("error")
            return
    ...
    def order_verify(self, notifyid):
        ...
        if response.body != "true":
            LOGGER.error("web_yymoon@%s", response.body)
            raise tornado.gen.Return(-1)
