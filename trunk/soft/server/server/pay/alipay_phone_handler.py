...
class NotifyHandler(base_handler.BaseHandler):
    ...
    def post(self):
        ...
        if  not util.alipy_rsa_verity(args, self.application.alipay_pubkey, self.application.alipay_prikey):
            LOGGER.error("yymoon@sign is not valid")
            self.response("error")
            return
        
        if args.get("trade_status") != "TRADE_SUCCESS" and args.get("trade_status") != "TRADE_FINISHED":
            LOGGER.error("yymoon@trade_status is not success")
            self.response("error")
            return

        try:
            pay = self.db.get("select id from pay_t where orderid = %s and pt = 'yymoon'", args.get("trade_no"))
            if pay:
                LOGGER.warning("yymoon@repeat send")
                self.response("success")
                return
        except Exception, e:
            LOGGER.error("yymoon@%s", e)
            self.response("error")
            return
        ...
        if check == 0:
            try:
                self.db.insert("insert into pay_t (id, openid, userid, itemid, orderid, amount, res, pt, dt) values (0, %s, %s, %s, %s, %s, %s, %s, NOW())",
                                            args.get("body"), 0, 0, args.get("trade_no"), int(float(args.get("total_fee")) * 100), 0, "yymoon")
            except Exception, e:
                LOGGER.error("yymoon@%s", e)
                self.response("error")
                return
        else:
            LOGGER.error("yymoon@verify error")
            self.response("error")
            return
    ...
    def order_verify(self, notifyid):
        ...
        if response.body != "true":
            LOGGER.error("yymoon@%s", response.body)
            raise tornado.gen.Return(-1)
